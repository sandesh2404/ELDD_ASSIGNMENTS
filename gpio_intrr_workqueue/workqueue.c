...
static int major;
static dev_t devno;
//1.create variable of workqueue_struct 
...
//3. workqueue handler write here
void workqueue_handle(struct work_struct *workqueue)
{   
    printk(KERN_INFO "%s: workqueue_handler() called\n", THIS_MODULE->name);
    //led_state = 1;
    int i=1;
    while(i<=10)
    {
...
static __init int bbb_gpio_init(void) {
   ...
   //2.initalize workqueue using variable declared globally
   INIT_WORK(&workqueue,workqueue_handle);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sandesh Thite <sandeshgpp@gmail.com>");
